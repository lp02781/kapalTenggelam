...
string typeRedTriangle 		= "UH8";
string typeRedTrapesium 	= "S1P";
string typeBlueTriangle 	= "G7C";
string typeBlueTrapesium 	= "A2X";
string typeYellowTriangle 	= "L6R";
string typeYellowTrapesium	= "JW3";
...
int main(int argc, char **argv){
	...
	image_transport::Publisher pub = it.advertise("camera/processing", 1, true);
...
void imageProcessing(){
	...
	for (redIndex = 0; redIndex < redContours.size(); redIndex++){
		...
		if (redApprox.size() == 3){
			...
			putText(Original, typeRedTriangle, Point(redX+50,redY+50), FONT_HERSHEY_DUPLEX, 1, Scalar(0,0,255), 2);
		...
		if (redApprox.size() == 4){
			...
			circle( Original, Point(redX, redY), 10, Scalar( 0, 0, 255 ), 2);//red circle
			rectangle(Original,Rect(redX-50,redY-50,100,100),Scalar(0,0,255),3,8,0);
			putText(Original, typeRedTrapesium, Point(redX+50,redY+50), FONT_HERSHEY_DUPLEX, 1, Scalar(0,0,255), 2);
	...
	for (blueIndex = 0; blueIndex < blueContours.size(); blueIndex++){
		...
		if (blueApprox.size() == 3){
			...
			putText(Original, typeBlueTriangle, Point(blueX+50, blueY+50), FONT_HERSHEY_DUPLEX, 1, Scalar(255,255,0), 2);
			...
			circle( Original, Point(blueX, blueY), 10, Scalar( 255, 0, 0 ), 2);//yellow circle      
			rectangle(Original,Rect(blueX-50,blueY-50,100,100),Scalar(255,0,0),3,8,0);
			putText(Original, typeBlueTrapesium, Point(blueX+50, blueY+50), FONT_HERSHEY_DUPLEX, 1, Scalar(255,255,0), 2);
			putText(Original, blueTrapesium, Point(textX, textY), FONT_HERSHEY_DUPLEX, 1, Scalar(255,255,0), 2);
			//cout<<"blue X = "<<blueX<<"\t\t"<<"blue Y = "<<blueY<<endl;
		}
	...
	for (yellowIndex = 0; yellowIndex < yellowContours.size(); yellowIndex++)
	{
		...
		if (yellowApprox.size() == 3){
			...
			putText(Original, typeYellowTriangle, Point(yellowX+50, yellowY+50), FONT_HERSHEY_DUPLEX, 1, Scalar(0,255,255), 2);
			putText(Original, yellowTriangle, Point(textX, textY), FONT_HERSHEY_DUPLEX, 1, Scalar(0,255,255), 2);
			...
			circle( Original, Point(yellowX, yellowY), 10, Scalar( 0, 255, 255 ), 2);//yellow circle
			rectangle(Original,Rect(yellowX-50,yellowY-50,100,100),Scalar(0,255,255),3,8,0);
			putText(Original, typeYellowTrapesium, Point(yellowX+50, yellowY+50), FONT_HERSHEY_DUPLEX, 1, Scalar(0,255,255), 2);
			putText(Original, yellowTrapesium, Point(textX, textY), FONT_HERSHEY_DUPLEX, 1, Scalar(0,255,255), 2);
			//cout<<"yellow X = "<<yellowX<<"\t\t"<<"yellow Y = "<<yellowY<<endl;
		}
	...
	allColour = redThreshold+blueThreshold+yellowThreshold;
	allContour = blueBW+redBW+yellowBW;
		
	imshow("image processing", Original);
	imshow("colour", allColour);
	imshow("contour", allContour);
}
